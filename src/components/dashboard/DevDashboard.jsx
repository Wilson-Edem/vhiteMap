...
import { doc, updateDoc, deleteDoc, collection, query, orderBy, limit, onSnapshot } from "firebase/firestore";
...
const DevDashboard = () => {
  ...
  const unsubscribeTrailRef = useRef(null);

  // --- Auto-update selected user when users list changes (live location updates) ---
  useEffect(() => {
    if (!selectedUser) return;
    const updatedUser = users.find(u => u.deviceUID === selectedUser.deviceUID);
    if (updatedUser) {
      // Only update if the lastLocation changed to avoid unnecessary re-renders
      if (JSON.stringify(updatedUser.lastLocation) !== JSON.stringify(selectedUser.lastLocation)) {
  ...
  // --- Live trail listener (onSnapshot) ---
  useEffect(() => {
    // Clean up previous listener
    if (unsubscribeTrailRef.current) {
      unsubscribeTrailRef.current();
      unsubscribeTrailRef.current = null;
    }
    ...
    const q = query(trailRef, orderBy("timestamp", "desc"), limit(500));

    unsubscribeTrailRef.current = onSnapshot(q, (snapshot) => {
      const points = [];
      ...
      console.log(`🔄 Trail updated: ${points.length} points`);
    }, (err) => {
      console.warn("Failed to listen to trail:", err);
      setUserTrail([]);
    });
        ...
        {selectedUser?.lastLocation ? (
          ...
          <div className="flex items-center justify-center h-full bg-dark/90 text-white/40">
            <div className="text-center">
              ...
              <p className="text-sm text-white/20 mt-1">to view their live location and trail</p>
            </div>
          </div>
        )}
      </div>
